...
- **Dev server**: `npm run dev` — runs `scripts/auto-cleanup.js` (frees port
  3000, clears stale `.next` state) then `next dev --turbo`.
- **Build**: `npm run build` — production build. It type-checks and **fails on
  type errors**; `ignoreBuildErrors` was removed in August 2026.
- **Typecheck**: `npm run typecheck`
- **Lint**: `npm run lint` (ESLint over `src` and `scripts`)
- **Start**: `npm start`

### When the dev server misbehaves

- `npm run kill:dev` — free port 3000
- `npm run kill:all` — kill Next processes, ports, caches, lock files
- `npm run dev:emergency` — last resort; `npm run dev:force` also restarts
- `npm run clean` — emergency cleanup + clear `node_modules/.cache`

See `docs/dev-server.md`.

### Important notes

- **Never run `npm run build` (or delete `.next`) while `npm run dev` is
  running.** Both write `.next`; doing so wedges the dev server, and the
  symptom looks like an application bug.
- The former "V2" dev-server tooling (process manager, lock manager, health
  monitor, dashboard, launchd watchdog) was removed in the August 2026
  cleanup, along with the `dev:safe`, `dev:monitored`, `dev:status`,
  `dev:watch`, `dev:cleanup` and `dev:logs` scripts. Do not reference them.
- **Next.js 16**: routing middleware lives in `src/proxy.ts`, not
  `middleware.ts`.
- The checkout sits inside iCloud Drive, which produces sync conflict copies.
  Moving it out is recommended.


## Architecture Overview

This is a fantasy football management application built with Next.js 16, featuring:
