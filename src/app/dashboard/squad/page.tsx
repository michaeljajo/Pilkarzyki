...
import { Trophy, Target, ArrowRight, Settings } from 'lucide-react'
...
export default async function SquadSelectorPage() {
  ...
  // If user is only in one league, redirect directly to that league's squad page
  if (leagues.length === 1) {
    redirect(`/dashboard/leagues/${leagues[0].id}/squad`)
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-[var(--foreground-secondary)]">Pick Team</span>
            </div>
        ...
        <div className="animate-fade-in-up">
          <div className="mb-8">
            ...
            <p className="text-[var(--foreground-secondary)]">
              Choose a league to pick your lineup
            </p>
          </div>

          {leagues.length === 0 ? (
            <div className="glass rounded-xl p-12 text-center">
              <Target size={48} className="mx-auto text-[var(--foreground-tertiary)] mb-4" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {leagues.map((league, index) => (
                <Link
                  key={league.id}
                  href={`/dashboard/leagues/${league.id}/squad`}
                  className="animate-fade-in-up"
                  style={{ animationDelay: `${index * 50}ms` }}
                >
                  <div className="glass rounded-xl p-6 hover-lift hover-glow group cursor-pointer h-full">
                    <div className="flex items-start justify-between mb-4">
                      <div className="w-12 h-12 rounded-lg bg-[var(--mineral-green)]/20 flex items-center justify-center group-hover:scale-110 transition-transform">
                        <Trophy size={24} className="text-[var(--mineral-green)]" />
                      </div>
                    ...
                    <div className="flex items-center justify-between mt-4 pt-4 border-t border-[var(--navy-border)]">
                      ...
                      <ArrowRight
                        size={20}
                        className="text-[var(--mineral-green)] group-hover:translate-x-1 transition-transform"
                      />
                    </div>
                  </div>
                </Link>
