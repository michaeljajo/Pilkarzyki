...
export default async function MidseasonDraftPage({ params }: Props) {
  ...
  if (league.is_active === false) redirect(`/dashboard/leagues/${leagueId}`)
  ...
  if (!isAdmin && !squad) redirect('/dashboard')
        ...
        <div className="max-w-[1400px] mx-auto" style={{ paddingLeft: '48px', paddingRight: '48px' }}>
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              ...
              <Link href={`/dashboard/leagues/${leagueId}`} className="text-gray-600 hover:text-gray-900">
                {league.name}
              </Link>
              <span className="text-gray-400">/</span>
