...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!userRecord) {
      console.log('Default Cup Lineup API - User not found in database, creating...')
      ...
      try {
        ...
        userRecord = newUser
        console.log('Default Cup Lineup API - User created successfully:', userRecord)
      } catch (clerkError) {
        console.error('Error fetching user from Clerk:', clerkError)

        // Fallback: create user with minimal information
        const fallbackEmail = userId.includes('@') ? userId : `${userId}@unknown.com`
        console.log('Default Cup Lineup API - Creating user with fallback info, email:', fallbackEmail)
        ...
        userRecord = newUser
        console.log('Default Cup Lineup API - Fallback user created successfully:', userRecord)
      }
    }
