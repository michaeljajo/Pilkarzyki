...
export function calculateTotalGameweeks(managerCount: number): number {
  if (managerCount < 2) {
    throw new Error('Need at least 2 managers for a league')
  }
  return 2 * (managerCount - 1)
}

export function calculateSeasonDuration(managerCount: number, intervalDays: number = 7): number {
  const totalGameweeks = calculateTotalGameweeks(managerCount)
...
export function generateGameweekSchedules(params: SeasonParameters): GameweekCalculationResult {
  ...
  // Validate date range
  const seasonDurationDays = calculateSeasonDuration(managerCount, gameweekIntervalDays)
...
export function validateSeasonFit(
  ...
  const requiredDays = calculateSeasonDuration(managerCount, intervalDays)
...
export function getRecommendedEndDate(startDate: Date, managerCount: number, intervalDays: number = 7): Date {
  const requiredDays = calculateSeasonDuration(managerCount, intervalDays)
